...
# 契约与单元测试（自动化测试）
tests/contract.sh --offline               # 离线半边检查（约 28s，不发网络包，覆盖 TUI 启动与 CLI 门控）
tests/contract.sh                         # 全量契约检查（约 90s，400+ 项检查，含真实端点探测）
tests/playback.sh                         # 真实 detached 播放器生命周期回归测试（约 88s）
...
- **测试绝不作为功能组件的替身**：测试必须驱动真实入口、解析真实信封、管理真实进程，严禁在 `tests/` 内自造 stub 代替实际组件。
