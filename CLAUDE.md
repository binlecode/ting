...
# 契约与单元测试（自动化测试）
tests/contract.sh --offline               # 离线半边检查（约 45s，不发网络包，覆盖 TUI 启动与 CLI 门控）
tests/contract.sh                         # 全量契约检查（约 140s，570 项检查，含真实端点探测）
tests/playback.sh                         # 真实 detached 播放器生命周期回归测试（约 105s，与全量契约并跑时实测）
