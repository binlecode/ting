...
```
   ...
   ├── USER_MANUAL.md             用户使用手册：人机 TUI 与 Agent CLI 双面完整操作指南
...
- **哪些源进本仓：判据是那五条，不是热度。** 一个源要住进 `shell/`，五条要全过 ——
  双半边都做得出来、零新增全局依赖、纯 bash 3.2、mpv 吃得下的原生直链、免登录且弱风控。
  全过的常驻主仓（今天 `yt` / `bili` / `ne`），受全量回归门禁守卫；**过不了的不是"以后再说"，
  是仓外**：它照样是一对文件、一份同样的契约，只是自己建仓、自己发版，放进 `$UT_ENGINE_DIR`
  ...
  ROADMAP 里那几条音源 NO（小宇宙、TME 系、汽水、Spotify）共享的正是这个形状 ——
  它们说的不是"这个源不好"，是"这个源过不了进仓判据"，而仓外那条路一直开着。
  这也是本套件不走另一个极端的理由：主程序零内置音源的微内核把"装上就能听"变成一次配置作业，
  而那正是这套东西的第一诉求（「定位与设计目标」）。
...
- **撤销是一条跨两个存储的规矩，不是每个键各打一个补丁。** 副本由**拥有对象的命令**在那次写
...
- **一条提示是帧的内容，不是一个模态。** 一个键做不成它被按下要做的事时，答案是标题行
  ...
  （`prompt_name` 的歌单名 —— 曾经的另一个，`y/N` 确认，已由 `z` 撤销取代）。这不是打磨：模态那一版让**下一个键同时
...
- **不可丢文本保底 8 格，修饰字段存活 12 格。** 核心主体文本（行标题、歌单名）截断下限统一钳位在
...
- **播放器里的 URL 嗅探** —— 当缺省 `--engine` 时，`t-play` 自动按 host 将合法的 HTTP(S) 或已知简写 URL 路由至匹配的引擎解析器（`yt` / `bili` / `ne`），显式 `--engine` 仍保持最高优先级，未知 host 保持回退至默认引擎（ARCH-cli-contract.md「命令规格」）。
