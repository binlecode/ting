...
```
 ...
 └──────────────────────────────────────────────────────────────────────────────
         ...
         ▼      —— 每一个都关在单一接缝后（「原语与接缝」）——     ▼
 ┌ 外部世界 ─────────────────────────────────────────────────────────────────────
 │   youtube.com · bilibili.com · music.163.com        音频输出 · 运行时控制
...
按名字引用（「Go 重写 NO」「卡片化 NO」），与这里的已落地决定无关。

### 套件 —— 平级动词，没有内核（接口：文件名 + argv，共享的是信封不是代码；「系统全景」、「命令拓扑」 的图）

- **没有内核**：十个平级成员各把自己的门、各调自己的原语，共享的是**信封**，
  不是代码。（「命令拓扑」）
...
- **一行结果就是一次可播调用，容器不是行**：不是可播调用的记录**在信封之前**就被引擎丢掉 ——
  ...
  回头看 url —— 把站点知识挪回了调用方。
  容器展开成清单的动词是 `<engine>-resolve --items`，三个引擎都有；创作者侧那种一次取不完的
...
- **不可丢文本保底 8 格，修饰字段存活 12 格。** 核心主体文本（行标题、确认框目标名、歌单名）截断下限统一钳位在
  `LAYOUT_MIN_BUDGET=8`（保证 2 汉字或 5 字符带省略号的可读下限），横幅阶梯亦以此为底丢弃副字段；修饰字段（hint/res/查询状态）
  低于 `LAYOUT_MIN_FIELD=12` 则整段丢弃以让出空间。预算绝不向物理宽度外向上强制 clamp，防止折行顶掉页眉。
  （ARCH-tui.md「宽度层」与「重排与分页」）
...
**十个命令，一层，无库。** 没有内核，也没有包装层。每个文件都是一个完整的、暴露在 PATH 上的
...
用的就是上面那十个名字。而 checkout **内部**的调用方（`tests/` 里的两个套件、技能）
...
- **封面「画成什么样」证不了；「发没发」证得了**（`ARCH-tui.md`「封面」）。
  ...
  以及画图有没有弄坏这个 TUI（tmux 段整体跑 `UT_IMAGE=on`，它抓到过三个真 bug）。
  **仍然证不了的是渲染**：位置、缩放、是不是那张图 —— 像素不是字节。
