...
```
   ...
   +----+----------------------------------------------------+-------------------------------+
        ...
        v                                                    v
   +-------------------------------------------+   +-----------------------------------------+
   | detached 独立进程组                       |   | 运行时临时目录: $TMPDIR/ting-<uid>/     |
...
8–10 行元数据，把 `ting` 的菜单顶进 scrollback、并挤掉进度条。修法分两部分，且要让窗口
OSD/OSC 与终端进度条都保持稳定：
...
在 `ting` 下两个进程逐字节抢输入，而 mpv 自己的默认键位占着 `[`/`]`（播放**速度**）、`9`/`0`、
...
```
   ...
   状态：${TMPDIR:-/tmp}/ting-$(id -u)/players/<id>.json（+ mpv-<id>.sock、mpv-<id>.log、
...
常态；`video`/`fast` 被接受，因为它们的 mpv 窗口是一个不需要这个终端的 GUI 面。`ting` 用同一
张清单校验 `-f`。

**`-d -j` 的信封带着 `sock` 与 `log`。** 它们本来就在状态文件里，而信封里没有它们时，客户端只能
从播放器**私有的**状态布局去**重建** socket 路径 —— 在第二个脚本里硬编码
`$TMPDIR/ting-$(id -u)/mpv-<id>.sock`，一旦播放器搬了状态目录就会静默失效（「运行时 IPC」）。
...
**IPC socket 是 `-d` 契约的一个公开部分（而且 `volume` 是活读的）。** `ting` 把每拍一次的**读**
...
已验证：在 `ting` 里按两下 `0`，把一个以 `--volume 0` 启动的播放器推到 `10`，`--status` 报的
就是 `10` —— 只读记录值的话它会永远报 `0`。

**`--status` 也交出 `sock` 与 `log`，理由与 `-d` 完全一样。** `-d` 的信封把 socket 交给了**发起
那次播放**的客户端。但接管一个不是自己起的播放器的客户端（`ting` 启动时的探测，ARCH-tui.md
「启动时接管」）从来没见过那个信封，它手上只有 `--status`。少了这一步投影，它唯一的出路就是照着
状态目录布局把 `$STATE_DIR/mpv-<id>.sock` 拼出来 —— 正是 `-d` 信封当初为了消灭而存在的那份重复，
而 `ting` 确实曾经硬编码过一次。这两个键状态文件本来就存着，所以补的是投影，不是新事实：
`--status` 的消费方要么多收到两个它不读的键，要么终于能控制它看见的播放器。
...
的人有 `ting`"），九个字段更不可能挤上去，而"到底在解码什么"的人机面是 `ting` 的详情块
...
### 队列 —— 一个正在被消费的播放列表（`--queue`、`--enqueue`、`--next`、五个 `--queue-*`）
...
```
   ${TMPDIR:-/tmp}/ting-$(id -u)/queue-<id>.json    {schema:1, pos, items:[{engine,url,…}]}
                                  lock-queue-<id>/   它自己的 mkdir 锁
                                  undo-queue-<owner>/  一个调用进程的撤销副本（「撤销副本」）
```
...
**队列文件的写者清单，以及为什么外面的人一个字节都不许碰它。** 写它的只有这个文件里的六处：
`queue_write_new`（`-d` 启动，fork 之前）、`queue_append`（`--enqueue`）、`queue_bump`（`--next`，
在父进程里）、`queue_advance_from`（**子进程**，每个曲目边界上，对 `pos` 的 CAS）、
`queue_edit`/`queue_clear_tail`（五个 `--queue-*`），以及 `do_queue_undo`（`--undo`，把副本写回）。**其中两个是并发的**：子进程的推进是任何
外部调用方都控制不了的一口钟。所以队列的改写只能是**动词**，不能是别的进程直接改那个文件 ——
`ting` 的队列视图（ARCH-tui.md「`u` 是第六个行源」）因此一次也不碰 `queue-<id>.json`：绕过
`lock_queue_state` 的写入，丢的不是自己的那一次改动，而是别人的。

**索引不是标识，所以改写要带身份别针。** 这是队列与歌单的分界：歌单没有并发写者，队列有。
一个调用方读到的索引是**对某一刻的猜测**，而队列随播放器一起消失 —— 删错了在任何地方都找不
回来。于是三个改写动词都要一个必需的 `--expect-url`，在**同一次锁内**先比 `items[INDEX].url`，
再比 `INDEX > pos`，两者都过才写（契约与退出码：ARCH-cli-contract.md「命令规格」「退出码」）。
- **被否：给队列文件加一个全局 `rev` 版本号。** 要在每个写点各加一次自增，而它能挡住的东西
  `--expect-url` 已经挡住了 —— 后者还额外告诉调用方**是哪一首**对不上。
- **一件让别针便宜的事实：`pos` 前进不移动 `items`，`--enqueue` 只追加也不移动。** 真正会重排
  `items` 的只有队列动词自己，而那正是别针拦下的那一类。
- **`--queue-jump` 把移动与推进放进同一次 jq 改写**，而不是"先 `--queue-mv` 再 `queue_bump`"：
  那是两次取锁，中间开一个窗口，一条曲目正好在那里结束就把 `pos` 推过刚填好的位置。写完之后
  它走 `--next` 的同一条收尾 —— `kill -USR1` 发给**子 shell 的 pid**，绝不发给进程组（理由见
  「进程组模型」：USR1 的默认处置是终止，发给进程组会把 mpv 从它该推进的那个循环底下杀掉）。
...
的。这一半曾是 `ting` 焦点卡上"前方队列块"的数据源；那张卡随视图塌缩删掉之后，TUI 只读它的
...
### 循环 —— 正本在磁盘上，socket 只负责让它立刻生效

播放器只认**一件事**：当前这首放完之后，是重来一遍（`one`）还是不重来（`off`）。它是 mpv 的
`loop-file` 属性，播放器验得到、改得动、答得出。

**为什么正本不在 mpv 里。** 队列每消费一项都重新走一遍 `play_url_directly → run_mpv`，
也就是**每首歌一个全新的 mpv 进程**；只活在进程里的属性会在第一个曲目边界上蒸发。所以
`--set-loop` 写的是 `players/<id>.json` 的 `loop`，而子进程在**装配 mpv 命令行的那一刻**
（`run_mpv` 里的 `refresh_loop_mode`）把它读回来。
...
由此它也不吃 `nc` 依赖门 —— 它向 `ipc_command` 要一个，要不到就耸耸肩。
...
**收听日志在 `one` 之下的口径**：`history_record` 在曲目结束时才落行，而循环中的曲目不结束 ——
所以循环一小时在停止时记一行，不是六十行。这与"一次收听一行"的既有口径一致，没有为循环开例外。
...
`ting` 的 `d`、`D`、`X` 不再先问一次 `(y/N)`，而是让用户在 3 秒内按 `z` 撤回（界面那一半在
ARCH-tui.md「可撤销取代预先确认」）。能撤回的东西放在哪、谁有权判定能不能撤回，是存储这一半，
...
```
   ...
   写 ... --owner PID -j  ------------------>  扫掉死 owner 的副本
                                               锁住对象
                                               读修改前 -> 写副本 {前, 后, 截止} -> 写修改后
                                               解锁
                          <------------------  信封 + undo.deadline
   --undo --owner PID -j  ------------------>  锁住对象
                                               未过截止？ 当前 == 副本的"后"（cmp -s）？
                                                 是 -> 写回"前"，删副本，退 0
                                                 否 -> 退 4（undo_expired / undo_stale）
   退出时 --undo --discard --owner PID  ---->  删副本
```

- **副本由拥有对象的命令在那次写自己的锁里写，顺序固定为"读修改前 → 写副本 → 写修改后"。**
  副本写不成，写就不做；副本写成而正文没写成（`die` 在两者之间），副本里的"修改后"与磁盘对不上，
  撤销必被拒绝 —— 一次失败的写永远不可能被"撤销"成一份重复。
- **存的是前后两份原文字节，不是时间戳，也不经过条目读取器。** `now_utc` 只到秒，同一秒两次写区分
  不开；而写回的是修改前的原文，于是 `added_at`、重复的 url、字段拼写都原样回来。"不存在"本身也是
  一种合法内容：`--add` 新建的列表撤销后不存在，`--del` 删掉的列表撤销后回来。
- **核对的是内容，不是名字。** `--undo` 在对象锁内逐字节比较当前内容与副本的"后"：别处在写与撤销
  之间改过它（另一次 `--add`、同名重建、改名后又有人写），就是 `undo_stale`，**什么都不写回**——
  改名涉及两个对象，只恢复还对得上的那一个会造出第三种谁也没要过的状态。
...
- **副本随 owner 一起死。** 正常退出时 owner 自己 `--discard`；`kill -9` 跳过了这一步，所以两个
  命令的**每一次**调用（任何动词，`--ls`、`--status` 也算）先扫一遍：owner 已经 `kill -0` 不到的
  副本一律删除，做法照搬 `reap_dead_players`。扫描按目录名进行，一个 glob 加每份副本一次 `kill -0`，
  不跑 jq。副本的实际寿命是"3 秒截止"与"owner 还活着"两者中较短的那个。
- **副本与对象住在同一层。** 歌单副本在状态目录里，与歌单同样持久；队列副本在
  `$TMPDIR/ting-<uid>/`，而且随播放器一起消失（`rm_player_files`，靠副本目录里的 `id` 文件认出
  以哪个播放器为对象）—— 一份比它要恢复的队列活得更久的副本，只能被拒绝。两者都放在各自对象的
  命名空间之外：`playlists/*.json` 是 `--ls` 的，`players/*.json` 是 `reap_dead_players` 的。
- **带 `--owner` 的队列写，锁超时即失败。** 队列锁在别处一直是尽力而为（超时就不带锁写），但副本
  若是在锁外取的，记下的是一条还会动的队列，信任它的撤销就会恢复错东西。所以带 `--owner` 的
  `--enqueue` / `--queue-rm` / `--queue-clear` 与 `--undo` 在超时时退 4 `locked`；不带的调用、
