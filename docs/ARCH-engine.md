...
**一行的封面是一个可以直接 GET 的 https URL，而不是站方给的那个形状** —— 这一条是分层，
...
**协议要补齐、要升级，这不是洁癖。** B 站那条协议相对的路径 `curl` 取不了 —— 补 `https:`
是"能取"与"不能取"的差别；网易云那条 http 站方同样用 https 供着同样的字节，而这个套件里
**不该有一处专门把连接降级**。两条都由 `tests/contract.sh` 的同一条不变量钉住，
...
| 端点 | 答什么 |
|---|---|
| `GET /api/search/get/web`（所有第三方客户端从前调的那个明文口） | `{"result":"35b174…"}` —— 一个十六进制密文块，不再是 JSON。**它死了** |
| `POST /weapi/cloudsearch/pc` | `code:200`，整页元数据：id、歌名、艺人、专辑、时长，以及 `fee` —— 一个请求，匿名 |

于是传输只能是 weapi，而 weapi 意味着两道 AES-128-CBC 加上一个 `encSecKey`。这是套件里**唯一**
一处自己实现密码学，所以它的每一个常量都要能被追问：

```
   step1  = base64( AES-128-CBC( payload_json, presetKey, iv ) )
   params = base64( AES-128-CBC( step1,        secretKey, iv ) )
   encSecKey = RSA_nopad( reverse(secretKey) )        # 编译期算好，运行时不算
```

**关键的一步是 `secretKey` 由调用方自己挑。** 浏览器每次请求现生成 16 个随机字符，而对面
...
再多就是对一个有风控的端点 burst，收益递减。翻页的判据读的是**整形之后**的行数，而整形只有
一个 jq 程序 —— 于是"要不要再取一页"和"发出去什么"永远不可能用两套不同的谓词。
...
一次搜索最多三个请求 —— 这两个数是引擎内部的成本策略，刻意不做成配置键。
...
`duration` 为 `null`，而信封上写着 `kind:"track"` —— `ut-play --engine yt -- <那条 url>` 不会
...
- **`lang` 恒为 `null`，这是诚实而不是缺功能。** 站方不给任何一条歌词打语言标签，而这个库不是
  ...
  也钉住了，免得日后有人"顺手补全"。同理没有 `--sub-lang`：一首歌一条歌词，没有可挑的东西。
- **`is_auto` 恒为 `false`，这个是知道的**：这个站的歌词是投稿的，响应里带着投稿者。
- **纯音乐要两个判据才认得出来，而这是实测逼出来的。** 站方文档里的 `nolyric` 标志几乎从不
  触发：2026-09-02 抽样八首纯音乐，**一首都没设**。它们返回的是一条真的 `lrc` —— 作曲/编曲的
  署名行，然后是站方自己的哨兵行 `纯音乐，请欣赏`。只读那个标志的实现会答 `status:"ok"` 外加
  三行署名，而一个被要求"总结歌词"的 agent 就会去总结某个人的名字。那句哨兵是站方写的 UI
...
`ut-play --start` 经由句柄往返，手工搭的队列会写它，而 `tests/contract.sh` 把"读得出 `t=601`、
