...
**管什么**：**交付给调用方（Agent 与人）的完整机器契约面** —— 10 个独立命令名及其 argv 参数、门控校验模型、退出码分类法（0/1/2+/4）、单行 JSON 信封（`-j`）Schema、detached 后台进程生命周期状态机，以及四级配置链（Flag > Env > User Config > Shipped Config）。
...
| 事项 | 归哪 |
|---|---|
| TUI 键位绑定、菜单导航、字符宽度计算与终端自绘 | [`ARCH-tui.md`](ARCH-tui.md) |
...
- **标志面由 `t-play --help` 陈述；这里只记取舍。** 颜色只有 `--color`（没有 `-c`）；
  ...
  **它是一道形状门，不是取值门，而这是 5 依赖下的必然。** 合法的颜色名是 ffmpeg 的那张表，
  ...
  拼错的名字则一路走到 mpv —— 实测那是每帧一行 `Cannot find color`、按默认色画、**退出码 0**。
  换言之这个选项没有「无效颜色」这一档语义失效（`4`），只有用法错（`1`）与透传（`0`）。
  **十六进制写 `0xRRGGBB`**；`#RRGGBB` 只有在标志上成立，写进配置文件会被当注释剪掉
  ...
  它说的是**即将启动**的播放器怎么行事，配上任何生命周期动词都退 1 并指路 `--set-loop`。
  它只管**重复**这一件事；"播完这首继续下一首"是队列不是循环，所以 `--loop sequential`
...
- **队列可以被审视和改写，而三个改写动词都必须带身份别针。**
  ...
  - **`--queue-jump` 是一次锁内完成的移动加推进**，不是 `--queue-mv` 后面跟一个 `--next`：
    ...
    于是播放器跳过了调用方点名要听的那首。
- **三个队列写可以留撤销副本，给点名了自己的调用方。** `--enqueue`、`--queue-rm`、
  `--queue-clear` 带 `--owner PID` 时，在同一次锁内留一份队列的前后副本，信封多一个
  `undo.deadline`；`--undo --owner PID` 在 3 秒内把队列写回，`--discard` 只删不写。不带
...
- **`-f` 在这里比在播放器上窄**：只收 `audio|video|fast`。播放是 detached 的，而 `ascii`/`viz`
  要一个终端 —— 那个终端已经被 TUI 自己占着（ARCH-player.md「状态机」）。
...
- **每次调用恰好一个动词**（七个，清单在 `usage()`）。没有 `--new`：`--add` 按需创建。`--index` 从 0 开始，跟 `--show` 印出来的一致，
  并且只属于 `--rm`（选择器配上别的动词就是退出 1，跟播放器上的 `--id` 一样）。
- **四个写可以留撤销副本，给点名了自己的调用方。** `--add`、`--rm`、`--del`、`--rename` 带
  `--owner PID` 时在写锁内留前后副本，信封多一个 `undo.deadline`；`--undo --owner PID` 在
  3 秒内写回，前提是每个涉及的列表还逐字节是那次写留下的样子。撤销信封报它恢复了哪张列表
  （`name`），`--rm` 的撤销再报那一项回到的位置（`index`），`--rename` 的撤销再报撤销前的名字
...
- **`kind` 与 `access` 同样是行上的必需键，且枚举封闭** ——
  ...
  **枚举装不下的新形态，按这两问映射到最近的值，映射记进 ARCH-engine.md —— 枚举不为它长大。**
...
- **`stream_urls` 是一个数组，视频在前。** 单流时一个元素；引擎的格式把一条纯视频轨与一条纯音频轨
  ...
  那正是 mpv 的 `ytdl_hook` 会用一份合成 EDL 顺手做掉的事（ARCH-player.md「模式 → 格式 → mpv」）。
...
- **`start_seconds` 是必需键**，取值 `null` 或一个**非负整数秒**：句柄要求从哪里开始，
  ...
  **`url` 里不带这个偏移**，两个键各回答一个问题：`url` 说这是哪个媒体，`start_seconds` 说从哪开始。
  ...
  （ARCH-engine.md「起播偏移」）。解析不出来的值（`?t=banana`）是 `null`，**不是错误**：
...
- **这个信封承诺的是"发不发"，此外什么都不承诺 —— 而"此外"是两件事。**
  ...
  因为那个账号不是大会员（ARCH-engine.md「先探后播」）。
...
`not_found | exists | invalid_name | invalid_input | locked | corrupt | undo_none | undo_expired | undo_stale`。
...
而这个设计决定是 ARCH-player.md「队列」。有一个后果是契约性的：
...
（ARCH-engine.md「解析」），从不是子串匹配。裸 id 那条路是同一条规则的另一个方向 ——
...
```
   ...
   1    用法/校验错误（`die` —— **只有调用本身**；工具装没装不落在这里，见下面的 2+ 与「依赖」）、
        ...
        一个不是 `off|one` 的 `--loop` / `--set-loop` 值（`sequential` 会被点名指向 `--queue -`）、
        `--loop` 配上一个生命周期动词（它是启动修饰符，改在跑的播放器是 `--set-loop`）、
   ...
   4    --set-volume / --pause / --resume / --seek / --seek-to / --set-loop / --enqueue /
        ...
        （ARCH-player.md「循环」）。
        带 `--owner` 的三个队列写与 `--undo` 还包括 `locked`：队列锁被占着时它们失败，
        而不是像别的队列写那样不带锁写下去，因为一份在锁外取的副本记的是一条还会动的队列。
        `--undo` 另有三个：`undo_none`（这个 owner 没有副本 —— 从没留过、已用掉、或它的
        播放器已停）、`undo_expired`（3 秒已过）、`undo_stale`（队列已不是那次写留下的样子，
        最常见的原因是播放器跨过了一个曲目边界；什么也不写回）。
        ...
        **t-playlist 也用它**，含义相同 —— 调用是格式良好的，而存储答不出来：
        ...
        `corrupt`（一个这个 build 读不了的文件），以及 `--undo` 的三个拒绝，与队列那边同名同义：
        `undo_none`、`undo_expired`、`undo_stale`（某个涉及的列表已被别处改过；什么也不写回）。1 留着表示它在这套套件里别处表示的东西：
...
- **就地改，只有值会动。** 键自己的空白、值与行尾 `#` 注释之间那段空白、以及注释本身
  都逐字节搬过去；文件里的空行、注释行、被注释掉的键、以及这十一个之外的每一行都不动。
